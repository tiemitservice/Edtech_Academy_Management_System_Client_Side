<template>
    <form @submit.prevent="handleSubmit" class="w-[320px]">
        ...
        <div class="p-4 text-start space-y-4">
            <div>
                <label for="start-time" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ $t('discount.name') }} <span class="text-red-500">*</span></label>
                <InputText :required="true" v-model="name" :placeholder="$t('discount.name')" class="w-full" />
            </div>
            <div>
                <label for="start-time" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ $t('discount.amount') }} <span class="text-red-500">*</span></label>
                <InputNumber :required="true" v-model="discount" :placeholder="$t('discount.amount')" class="w-full" />
            </div>
        </div>
